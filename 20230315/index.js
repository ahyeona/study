...
// 길이가 변경되는 상황이 발생했는데
// 반복문은 7번 돌아가기 때문에
// 배열의 길이가 8로 늘어나게 되면 마지막 값을 가져오지 못함
// for (let i = 0; i < 7; i++) {
//     // 0 1 2 3 4 5 6
//     console.log(b[i]);
//     // 값 1 2 3 4 5 6 7
...
// 그러면 반복문을 정상적으로 배열의 길이만큼 반복시킬 수 있다.
...
// const 한 번 선언하면 값을 변경할 수 없다. 재선언도 불가능
// 상수의 값
// 객체는 키와 값이 있다.
//
const obj = {
...
// 딸기라는 값의 인덱스를 알고싶음
console.log(return3); // 1

// 배열 메소드 find
// 검색할 때 사용할 함수
// 함수 반환값이 true가 나오는 함수를 넣어주고

// find라는 함수는 매개변수로 함수로 전달받는다.
let return4 = arr.find(function (i) {
  // 익명함수
  // 배열의 값이 순서대로 들어온다.
  // i 매개변수에
  console.log("item : " + i);
  // return 값이 true가 반환되면 find 함수에서 해당 아이템을 반환해준다.
  return i === "딸기";
  // return true;
});
...
// filter 검색창 같은 알고리즘 구현할 때 사용할 것 같음

// 원하는 값을 모두 찾아서 반환해준다.
console.log(return6); // ['이사과', '이포도']
...
console.log(return7); // [true, false, true]
...
// 배열의 길이만큼 반복하면서 값을 뽑아줌.
// 값을 바로 사용할 수 있다.
// 아이템을 순차적으로 뽑아준다.

// 지금까지 for문에 증가하는 값을 배열의 인덱스로 줘서 값을 확인했는데

// 배열 메소드 join
// join 함수는 매개변수로 문자열을 전달받음
...
// 문자열을 배열로 변경
// split 함수
// split 함수의 매개변수로 문자열 값을 자를 문자값을 넣어주면 됨
