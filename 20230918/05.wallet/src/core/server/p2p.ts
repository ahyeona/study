...
enum MessageType {
    // 알기 쉽게 사용하려고
    // 0, 1, 2 상태를 지정한다고 했을 때
    // 마지막 블록을 요청할 때
    lastBlock = 0, // 문자로 해도 되는데 오타가 발생할 수 있어서 number로 최대한 오류가 없게
    // 전체 체인을 요청할 때
    allBlock = 1,
    // 블록이 추가돼서 알려줄 때
    addBlock = 2
...
class P2P extends Chain {
    ...
    // private sockets : Array<WebSocket> // 연결된 socket들 확인
    ...
    connectSocket(socket : any, type? : MessageType) : void {
        ...
        // socket에는 고유의 포트가 들어있는 상태. 충돌 방지를 위해 애플리케이션 or 서비스 연결을 하면
